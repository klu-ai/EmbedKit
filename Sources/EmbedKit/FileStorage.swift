...
/// A file‑based storage provider that implements Storage using JSON files.
...
public class FileStorage: Storage {
    ...
    public func loadDocuments() async throws -> [Document] {
        return Array(documents.values)
    }

    /// Returns all documents currently in the in-memory cache.
    public func getAllDocuments() -> [Document] {
        return Array(documents.values)
    }

    /// Returns the normalized embedding for a specific document.
    public func getNormalizedEmbedding(for id: UUID) -> [Float]? {
        return normalizedEmbeddings[id]
    }
    ...
    public func updateDocument(_ document: Document) async throws {
        try await saveDocument(document)
    }

    /// Resets the storage by deleting all files and clearing caches.
